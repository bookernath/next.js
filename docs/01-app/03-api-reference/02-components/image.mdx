...
A [Data URL](https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/Data_URIs) or external URL to
...
If the image is dynamic or remote, you must provide `blurDataURL` yourself. You can use:

- **Data URLs**: Inline base64-encoded images (recommended for best performance)
  - [A online tool like png-pixel.com](https://png-pixel.com)
  - [A library like Plaiceholder](https://github.com/joe-bell/plaiceholder)
- **External URLs**: Direct URLs to small placeholder images (e.g., `https://cdn.example.com/blur.jpg`)
  - Use small images (10px or less) from a fast CDN
  - Note: External URLs require an additional network request and must respect CORS policies
  - Consider potential performance impact compared to Data URLs
