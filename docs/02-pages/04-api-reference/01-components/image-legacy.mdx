...
Can be a base64-encoded Data URL or an external URL (e.g., `https://cdn.example.com/blur.jpg`). The image will be enlarged and blurred, so a very small image (10px or less) is recommended. Including larger images as placeholders may harm your application performance.

**Note**: External URLs require an additional network request and must respect CORS policies. Data URLs are recommended for best performance.
